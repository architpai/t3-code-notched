...
export function dockAt(area: Rect, point: { x: number; y: number }): Dock {
  const nearLeft = Math.abs(point.x - area.x) <= 64;
  const nearRight = Math.abs(point.x - area.x - area.width) <= 64;
  const nearTop = Math.abs(point.y - area.y) <= 64;
  const nearBottom = Math.abs(point.y - area.y - area.height) <= 64;
  ...
  const distances: [Edge, number][] = [
    ...
    ["bottom", Math.abs(point.y - area.y - area.height)],
    ["left", Math.abs(point.x - area.x)],
    ["right", Math.abs(point.x - area.x - area.width)],
  ];
  const edge = corner
...
export function panelBounds(
  ...
  const width = Math.min(
    Math.max(
      expanded ? 420 : dock.corner ? 84 : side ? 48 : 360,
  ...
  const height = Math.min(
    Math.max(
      !expanded && dock.corner ? 84 : side ? 160 : 40,
